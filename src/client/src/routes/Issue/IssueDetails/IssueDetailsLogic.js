...
const IssueDetailsLogic = () => {
    ...
    async function fetchData() {
      await axios.all([getIssue(), getUsers()]).then(
        axios.spread((issue, users) => {
          setIssue(issue.data);
          setComments(issue.data.comments);
          setUsers(users.data);
          setLoading(false);
  ...
  const onCommentDelete = (row) => {
    deleteRequest("/comment/delete", { id: row.id }).then((response) => {
      setIssue({
        ...issue,
  ...
  const onCommentSubmit = (event, comment) => {
    ...
    if (comment.length > 5) {
      ...
      post("/comment/new", requestBody).then((response) => {
  ...
  const onChangePriority = (event) => {
    ...
    post(
      "/issue/edit/priority",
      {},
      { issueId: issue.id, priority: value }
  ...
  const detailsContent = () => {
    return [
      {
        title: "Assigned",
        body: issue.users.map((user, index) => (
          <div className="d-flex flex-row" key={index}>
            <span className="flex-grow-1 d-flex align-items-center">
              {user.name + " " + user.lastName}
            </span>
            <button onClick={() => unassignUser(user)} className="btn">
              -
            </button>
          </div>
        )),
